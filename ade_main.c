...
main (int argc, char **argv)
{
  ...
  while ((c = getopt (argc, argv, "b:c:h")) != EOF)
    switch (c)
      {
      case 'b':
	strcpy (bootdisk, optarg);
	break;
      case 'c':
	strcpy (rc_config, optarg);
	break;
      case '?':
	usage ();
	exit (1);
...
/*  printf (" config file = \"%s\"   bootdisk = \"%s\"\n", rc_config, bootdisk); */
...
/* tell the shared library black_box what sort of machine we'll be using and where our user routines are */
  ...
  cpux->pc = 0;
  term_type = 0;
  term_width = TERM_WIDTH_STD;	/* set default terminal width to 80 unless specify 'wide' (132char) */
  term_height = TERM_HEIGHT_STD;	/* set default terminal height to 25 unless specify 'high'(60 char) */
  get_environment ();
...
*/
  ...
  /*insert 'standard' peripheral cards into normal slots. 'FALSE' for silent */
  add_slot_card ("PIO", 1, FALSE);
  add_slot_card ("SIO", 2, FALSE);
  add_slot_card ("HDC", 6, FALSE);


/* lets have a look at the config file */
  if (!strlen (rc_config))
    {
      fetch_config (MACHINE_CONFIG_NAME, RC_GLOBAL);
    }
  ...
  error = prepare_emulator (argc, argv);
  if (error)
    {
      terminate (error);
    }
...
/******************************************************************/
  switch (p_hi)
    ...
    case 9:
      xlog (MOTHERBOARD, "mb_in(%02X): ERROR. OUTPUT ONLY PORT: Start Scan register OPs\n", p_hi);
      data = 0xaa;
      break;

    case 0x0A:
      xlog (MOTHERBOARD, "mb_in(%02X): ERROR. OUTPUT ONLY PORT: Memory Mapping Register OPs\n", p_hi);
      data = 0xaa;
      break;

    case 0x0B:
      xlog (MOTHERBOARD, "mb_in(%02X): ERROR. OUTPUT ONLY PORT: Clear Display Flag\n", p_hi);
      display_flag = FALSE;
      data = 0xaa;
      break;

    case 0x0C:
      xlog (MOTHERBOARD, "mb_in(%02X): ERROR. OUTPUT ONLY PORT: Clear z80 NMI Flag\n", p_hi);
      data = 0xaa;
      break;
    ...
    case 0x0F:
      xlog (MOTHERBOARD, "mb_in(%02X): ERROR-OUTPUT ONLY: Output I/O control reg\n", p_hi);
      data = 0xaa;
      break;

    default:
      data = 0xff;
      xlog (MOTHERBOARD, "adv_main: ERROR. Input from NON-STANDARD HORIZON Port[%02X]:  Data:%02X \n",
	    port, data);
      break;
...
/******************************************************************/
  switch (p_hi)
    ...
    case 7:
      xlog (MOTHERBOARD, "mb_out(%02X): [INPUT-ONLY = NOP] Get Board ID Code[%02X]\n", p_hi, data);
      break;
	...
	case 03:
      ...
      xlog (MOTHERBOARD, "mb_out(%02X): [OUT] Memory Mapping Register OPs[%02X]\n", p_hi, data);
      if (!(data & 0x080))
	{			/* hi-bit zero = set ram page */
	  xlog (MOTHERBOARD, "       p_lo=%2d   MAP REGISTER %02d: RAM %s: Data=%02X page=%d\n",
		p_lo, p_lo, extra_info, data, data & 0x07);
	  map_ram_page (p_lo, data & 0x07);
	  switch (p_lo)
	    {
	    case 0:
      ...
      else
	{
	  if ((data & 0x84) == 0x84)
	    {
	      xlog (MOTHERBOARD, "       p_lo=%d   MAP REGISTER %d %s: BOOT PROM: Data=%02X \n", p_lo, p_lo,
		    extra_info, data);
	      set_boot_prom_active (p_lo);
	    }
	  if ((data & 0x06) == 0)
	    {
	      xlog (MOTHERBOARD, "       p_lo=%d   MAP REGISTER %d: DISPLAY RAM %s: Data=%02X page=%d\n",
		    p_lo, p_lo, extra_info, data, 8 + (data & 0x01));
	      set_display_ram_page (p_lo, 8 + (data & 0x01));
    ...
    case 0x0D:
      xlog (MOTHERBOARD, "mb_out(%02X): NOT VALID Input (I/O status reg 2) [%02X]\n", p_hi, data);
      xlog (MOTHERBOARD, "         ###   NOT IMPLEMENTED   ###\n");
      break;

    case 0x0E:
      xlog (MOTHERBOARD, "mb_out(%02X): NOT VALID Input (I/O status reg 1) [%02X]\n", p_hi, data);
      xlog (MOTHERBOARD, "         ###   NOT IMPLEMENTED   ###\n");
      break;
    ...
    default:
      xlog (ALL, "nse_main: ERROR. Output To NON-STANDARD Advantage Port[%02X]:  Data:%02X    %c\n",
	    port, data, prn);
      break;
    }
}
...
int
disk_manager (int diskop, int diskno, char *diskname, int extra1, int extra2)
{
  int status;
  int q;

  q = machine_floppy_max;
  status = 0;
  switch (diskop)
    {
      ...
      if (diskname == NULL)
	{
	  if (nsd[diskno - 1].fdfn != NULL)
	    {
	      strcpy (diskname, nsd[diskno - 1].fdfn);
	    }
	  else
	    {
	      diskname[0] = '\0';
      ...
      xlog (FDC, " Disk no. %d, Disk name: %s Switch: %d\n", diskno, diskname, extra1);
      if ((diskno > 0) && (diskno < (q + 1)))
	{
	  floppy_mount (diskno, diskname, extra1);
	}
      if ((diskno > 100) && (diskno < (MACHINE_HD_MAX + 101)))
	{
	  hdmount (diskno - 101, diskname, extra1);
	}
    ...
    case INITIALISE:
      xlog (FDC, "DISKOP - INITIALISE - initialise all floppy disk drives and leave unmounted\n");
      initialise_floppies ();
      if (MACHINE_HD_MAX)
	{
...
set_io_control_register (int io_ctl)
{
  ...
  cmd = (io_ctl & 0x07);
  io_control_reg = cmd;		/* save command so we know what results signify */
  ...
  switch (cmd)
    ...
    case 2:
      ...
      break;
    case 3:
      xlog (CMD, "set_io_control_register: CMD 3: Complement KB MI 4-key-Reset Enable Flag\n");
      /***/
      break;
    case 4:
  ...
  if (io_ctl & 0x08)
  ...
  else
    {
      ...
      xlog (FDC, "    08: acquire_mode = LOW         sector= %d  (==>  track = %d  sector %d)\n",
	    floppy->fd_sector_num, floppy->fd_track_num, (floppy->fd_sector_num + 1) % 10);
      floppy->fd_acquire_mode_prev = floppy->fd_acquire_mode;
    }
  ...
  if (io_ctl & 0x80)
    {
      xlog (CMD, "set_io_control_register: ******** Enable Display Interrupt &&&&&&&&&\n");
    }
...
get_status_reg_2 ()
{
  ...
  if (!ascii)
      ...
      if ((conbuffstat ()))
	{
	  ...
	  if (z)
	    {
	      kbd_char = z;
	      xlog (KEYB, "get_status_reg_2: @@@@@@@@@ KBD CHAR:  = %02X [%c]\n", kbd_char, kbd_char);
	      kbd_data_flag = 1;
	      char_overrun = 0;
  ...
  switch (io_control_reg)
      ...
      else
	{
	  ...
	  xlog (CMD, "get_status_reg_2: 'Sector Number' is  0E h   (= MOTOR OFF)\n");
	}
      break;
    case 1:
      ...
      if (prefix_toggle)
	{
	  if (four_key_reset_enable_flag)
	    {
	      status |= 1;
	      xlog (CMD, "get_status_reg_2: Keyboard NMI 4_key_reset_enable Flag is TRUE\n");
	    }
	  else
	    {
	      xlog (CMD, "get_status_reg_2: Keyboard NMI 4_key_reset_enable Flag is FALSE\n");
	    }
	  prefix_toggle = 0;
	}
      else
