...
void set_1_dib ();
void calc_sectors ();
void calc_allocation_factor ();
void calc_efficient_filler ();
void sorry_max ();
  ...
  if (unit)
	...
	case 4:
	  ...
	  while ((allocation_factor != 1) &&
		 (allocation_factor != 2) &&
		 (allocation_factor != 4) && (allocation_factor != 8) && (allocation_factor != 16))
	    {
	...
	("\nFile is:  %d bytes, %d blocks, %d sectors, %d DIBs, allocation factor = %d, %6.1f KB \n\n",
	 bytes, blocks, sectors, dibs, allocation_factor, kilobytes);
      printf ("    HDOS Command Line:     CR FILENAME[[,ACCOUNT],DISK_UNIT]  %d", blocks);
...
set_1_dib ()
{
  ...
  printf ("  The minimum filesize occupies one 16-sector DIB (15 sectors plus 1 sector for DIB-Directory)\n");
  printf ("  As the filesize required is less than that, the space taken up\n");
...
banner ()
{
  ...
  printf ("\n");
  printf ("A North Star Floppy Disk file is restricted to a maximum length of 66\n");
  printf ("tracks on a DQ disk, or 660 sectors, 1320 blocks, 330 kilobytes.\n");
  printf ("\n");
  printf ("A North Star Hard-Disk file is made from 'hunks' containing multiple\n");
  printf ("sectors.  These 'hunks' were originally so-named by North Star, but later\n");
  printf ("this name was changed to 'DIBs'.\n");
  printf ("\n");
  printf ("Each DIB ('Data Incremental Block', similar to 'clusters', 'extents', etc. \n");
  printf ("in other operating systems) contains a multiple of 16 sectors.  There can be\n");
  printf ("a maximum of 128 DIBs per file.\n");
  printf ("\n");
  printf ("Since this could really restrict the maximum size of a file, a power-of-2\n");
  printf ("factor can be applied to 16 giving 16, 32, 64, 128, or even up to 256 sectors\n");
  printf ("per DIB.  Consequently, it becomes possible to produce a file which can go up\n");
  printf ("to the maximum allowable file-size on a hard-disk: 65,535 blocks, 32,768\n");
  printf ("sectors or 16.384 megabytes.\n");
  printf ("\n");
  printf ("Each file contains its own internal DIB-directory, which takes up the first\n");
  printf ("sector of the file itself.  Keep this 'loss' of the first file sector in\n");
  printf ("mind when creating your files on the hard-disk.  The Hard-Disk Directory (or\n");
  printf ("Index) merely tells HDOS where the file's first sector with its\n");
