...
void put_int16 (BYTE * cptr, int v);
void new_system_account_entry ();
int new_transient_file_entry ();
void fill_transient_sectors ();
void setbit ();
void rw_logical ();
#define S_READ 1
#define S_WRITE 2
...
#define MHVERSION "3.0"
FILE *hd;

#include "transient.c"		/* data with code of the TRANSIENT file */
...
main ()
{
  ...
  struct hd_type ns_hd_5x0 = { "SG5A", 1, 0, 9792, 16, 4, 128, 240, 16, 4, 153, 64, 128, 152 };
  struct hd_type ns_hd_5x1 = { "TN5A", 2, 0, 9792, 16, 4, 128, 240, 16, 4, 153, 66, 128, 152 };
  struct hd_type ns_hd_5x2 = { "MS5B", 2, 0, 9792, 16, 4, 128, 0, 16, 2, 306, 0, 0, 336 };
  struct hd_type ns_hd_5x3 = { "RD5B", 2, 0, 9792, 16, 4, 128, 0, 16, 2, 306, 0, 132, 319 };
  struct hd_type ns_hd_5x4 = { "SG5B", 2, 0, 9792, 16, 4, 128, 0, 16, 2, 306, 128, 0, 305 };
  struct hd_type ns_hd_5x5 = { "TN5B", 2, 0, 9792, 16, 4, 128, 0, 16, 2, 306, 0, 128, 305 };
  struct hd_type ns_hd_5x6 = { "CM10E", 2, 0, 19584, 16, 4, 128, 0, 16, 2, 612, 256, 0, 650 };
  struct hd_type ns_hd_5x7 = { "MS10E", 2, 0, 19584, 16, 4, 128, 0, 16, 2, 612, 128, 0, 656 };
  struct hd_type ns_hd_5x8 = { "CM15C", 2, 0, 29376, 16, 4, 128, 0, 16, 6, 306, 306, 128, 305 };
  struct hd_type ns_hd_5x9 = { "SG15C", 2, 0, 29376, 16, 4, 128, 0, 16, 6, 306, 128, 0, 305 };
  struct hd_type ns_hd_5x10 = { "RD15C", 2, 0, 29376, 16, 4, 128, 0, 16, 6, 306, 0, 132, 319 };
  struct hd_type ns_hd_5x11 = { "TN15C", 2, 0, 29376, 16, 4, 128, 0, 16, 6, 306, 0, 128, 305 };
  struct hd_type ns_hd_5x12 = { "MS15D", 2, 0, 30720, 16, 4, 128, 0, 16, 4, 480, 240, 128, 522 };
  struct hd_type ns_hd_5x13 = { "MS15E", 2, 0, 29376, 16, 4, 128, 0, 16, 4, 459, 0, 128, 522 };
  struct hd_type ns_hd_5x14 = { "CM20E", 2, 0, 39168, 16, 4, 128, 0, 16, 4, 612, 256, 0, 650 };
  struct hd_type ns_hd_5x15 = { "MS20E", 2, 0, 39168, 16, 4, 128, 0, 16, 4, 612, 0, 0, 656 };
  struct hd_type ns_hd_5x16 = { "RD20E", 2, 0, 39168, 16, 4, 128, 0, 16, 4, 612, 0, 0, 639 };
  struct hd_type ns_hd_5x17 = { "MS30D", 2, 0, 58752, 16, 4, 128, 0, 16, 8, 459, 0, 0, 522 };
  struct hd_type ns_hd_5x18 = { "CM30E", 2, 0, 58752, 16, 4, 128, 0, 16, 6, 612, 256, 0, 650 };
  struct hd_type ns_hd_5x19 = { "MS30E", 2, 0, 58752, 16, 4, 128, 0, 16, 6, 612, 0, 0, 656 };
  struct hd_type ns_hd_5x20 = { "RD30E", 2, 0, 58752, 16, 4, 128, 0, 16, 6, 612, 0, 0, 639 };
  ...
  printf ("\n\n                         === mkhd ===\n");
  printf ("                         Version  %s\n\n", MHVERSION);
  printf ("Prepares a \"Standard\" 5-inch Hard-Disk Imagefile for use with North Star\n");
  printf ("Horizon Emulator (nse) and Advantage Emulator (ade) running HDOS.\n\n");
  printf ("Disk-image sizes available range from 5 MB to 30MB. \n\n");

  printf ("                                  Usable    Usable  Shipping  Total   Total \n");
  printf ("No.   Type   Rev. Cylinders Heads Sectors  Capacity Cylinder Sectors Capacity\n\n");
  ...
  for (i = 0; i < 21; i++)
      ...
      else
	{
      ...
      printf ("%2d   %5s   %c.%c   %4d     %2d    %5d    %5.2f M    %d     %5d  %5.2f M\n",
	      i + 1, ns_hd_5x[i]->hd_type_code, ns_hd_5x[i]->hd_major_rev + '0',
	      ns_hd_5x[i]->hd_minor_rev + '0',
	      ns_hd_5x[i]->hd_max_cylinders,
	      ns_hd_5x[i]->hd_heads_number,
	      ns_hd_5x[i]->hd_total_sectors, cap[i], ship_cyl[i], tsectors[i], tcap[i]);
    }
  printf ("\n\n");
  typeok = 0;
  nameok = 0;
...
/* now fill the first-sector area with required stuff*/
  ...
  *(sector_img_ptr + DLMINOR) = ns_hd_5x[x]->hd_minor_rev;
  put_int16 (sector_img_ptr + DLDSZE, (ns_hd_5x[x]->hd_total_sectors - ns_hd_5x[x]->hd_reserved_sectors));
...
/* write disk-image to file */
  ...
  printf ("Do you want to include the SYSTEM account and TRANSIENT file?\n");
  printf ("  (Not required for Advantage, only Horizon)            (Y/n) ");
  fgets (answer, 18, stdin);
...
void
rw_logical (unsigned char *bptr, int sect_num, int mode)
{
  int ls;
  int phys;
  int shift_track;
...
/*  filesize = 24; */
  ...
  strcpy (nambuff, "TRANSIENT");	/*chop '/' from unixname */
...
fill_transient_sectors ()
{
  ...
  for (i = 0; i < 0x18; i++)
    {
      rw_logical (&(transient_bin[i * 0x200]), (0x31 + i), S_WRITE);
