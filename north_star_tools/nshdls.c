...
ltype (int tnum)
{
  ...
  static char *t0 = "Non-Specific File Type     ";
  static char *t1 = "NSDOS Executable           ";
  static char *t2 = "HBASIC program             ";
  static char *t3 = "HBASIC data file           ";
  static char *t4 = "Backup Diskette Index      ";
  static char *t5 = "Hard Disk Backup Data      ";
  static char *t6 = "CP/M Workfile              ";
  static char *t7 = "CP/M Virtual Disk File     ";
  static char *t18 = "ASP Sequential Access File ";
  static char *t19 = "ASP Random Acess File      ";
  static char *tx = "Unknown File Type          ";
  static char *tacct = "ACCOUNT ";
  if (!xdir.accid_nshdd)
    {
...
*/
  if (dibno == 0xffff)
    ...
    {
      sprintf (xdir.d_image_offset_str_nshdd, "  %08X   ", get_int16 (&(directory[24])) * 0x2000);
    }
...
display_dirent ()
{
  ...
  if (header_flag)
    {
      ...
      if (show_extra)
	{
	  strcat (xline, "Attr. inLast dibNum/Sectors ndib0_Addr  ndib0_D_Off. TypeData DirOffset DirType");
	}
      printf ("%s\n\n", xline);
      header_flag = 0;
    }
  ...
  if (erased)
    {
      strncpy ((typestring + 2), " **  DELETED  ** ", 17);
    }

  sprintf (xline,
	   "%-14s %4d    %4d blk     %-14s%2d     %s",
	   xdir.fname_nshdd, xdir.filesize_nshdd, xdir.filesize_nshdd * 2, account_name[xdir.accid_nshdd],
	   xdir.filetype_nshdd, typestring);
  if (!xdir.accid_nshdd)
    {				/*account name/number entry */
      sprintf (xxline, "(%02d) \"%s\"", xdir.attributes_nshdd, account_name[xdir.attributes_nshdd]);
  ...
  if (show_extra)
    {
      sprintf (xxline, " %04X     %04X    %d     %3d  %s%s %04X.%02X   %06X    %d",
	       xdir.attributes_nshdd, xdir.lsbytes_nshdd, xdir.dibsize_nshdd,
	       xdir.dibsize_sects_nshdd, xdir.ndib0_str_nshdd, xdir.d_image_offset_str_nshdd,
	       xdir.typedata_nshdd, xdir.typedata3_nshdd, xdir.addr_nshdd, xdir.dirtype_nshdd);
      strcat (xline, xxline);
    }
  printf ("%s\n", xline);
}
...
usage (char *prg)
{
  ...
  printf ("            Option:  -l       Show extra directory information\n\n");
}
