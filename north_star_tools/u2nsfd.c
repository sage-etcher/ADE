...
void usage (), getargs (), dlook (), set_density ();
void cleanbuff (), replacefile (), appendfile ();
void put_int16 ();
...
/* get NS-filename from first part of unix filename, if delimited by '_' */
  while ((d < NAMEMAX) && (nambuff[j] != '\0') && (nambuff[j] != '_') && (nambuff[j] != '.'))
    {
      if (nambuff[j] == ' ')
	nambuff[j] = 'X';
...
/* try to get type if included in unix name */
  ...
  printf ("%8s to be added to %10s  %3d blocks   type %d  ", znambuff, argv[2], nflen, type);
  if (type == 1)
...
usage ()
{
  ...
  printf ("Usage: u2ns <unix-filename> <double-density-NS-disk-image-file>\n\n");
}
