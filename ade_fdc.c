...
floppy_mount (int disk, const char *filename, int readonly)
{
  ...
  if (filename == NULL)
    {
      xmonprint (1, 2, "No Floppy Image File Specified");
      xmonscroll ();
      return 0;
    }
  ...
  *fdfilename = '\0';		/* make it an empty string */
  ...
  if ((st.st_mode & __S_IFMT) != __S_IFREG)
    {
      xmonprint (1, 2, "   No File '%s'", fdfilename);
      xmonscroll ();
      return 0;
    }
  ...
  if (floppy->fdd != NULL)
    {
      xmonprint (1, 2, "   *** Floppy %d is already mounted. Unmount first! ***", disk);
      xmonscroll ();
    }
  else
    {
	...
	{
	  xmonprint (1, 2, "   *** Can't Mount Floppy %d, File < %s > ***", disk, fdfilename);
	  xmonscroll ();
	}
      else
	{
	  ...
	  if (!error)
	    {
	      xmonprint (1, 2, " FLOPPY %d, <%s> %ld K", disk, fdfilename, flen / 1024L);
	      xmonscroll ();
	    }
	  else
	    {
	      xmonprint (1, 2, " FLOPPY %d, <%s> Unknown bytes", disk, fdfilename);
	      xmonscroll ();
	    }
...
show_mount_table (int disk)
{
  ...
  for (disk = 1; disk <= q; disk++)
    {
      if (nsd[disk - 1].fdd == NULL)
	{
	  xmonprint (1, 2, "  Floppy    %d is   ** not mounted. **", disk);
	  xmonscroll (1);
	}
      else
	{
	  xmonprint (1, 2, "  Floppy    %d is <%s>", disk, nsd[disk - 1].fdfn);
	  xmonscroll (1);
	}
...
umount (int disk)
{
  ...
  nsd[disk - 1].fdd = NULL;
  xmonprint (1, 2, "  Floppy    %d  <%s> now unmounted.", disk, nsd[disk - 1].fdfn);
  xmonscroll ();
  nsd[disk - 1].fdfn[0] = '\0';
  ...
  switch (port_lo)
      ...
      if ((prn < 0x20) || (prn > 0x7e))
	{
      ...
      xlog (FDC,
	    ...
	    floppy->fd_track_num, floppy->fd_sector_num, floppy->fd_byte_ptr, data, prn);
      floppy->fd_byte_ptr++;
...
fdc_out (BYTE port_lo, BYTE data)
{
  ...
  switch (port_lo)
    ...
    case 2:
      ...
      xlog (FDC, "FLOPPY DISK CONTROLLER INITIALISED, TURN ON FLOPPY MOTORS \n");
...
load_drive_control_register (int dsk_ctl)
{
  ...
  if (dsk_ctl & 0x20)
    {
      xlog (FDC, "load_drive_control_register: [20] Step Direction 1 = Inwards\n");
      floppy->fd_step_direction = 1;
    }
  else
    {
      xlog (FDC, "load_drive_control_register: [20] Step Direction 0 = Outwards\n");
      floppy->fd_step_direction = 0;
    }
...
write_sector_to_disk ()
{
  ...
  xlog (FDC, " WRITE Track %d SECTOR %d  Side %d TO DISK %c: \n",
	floppy->fd_track_num, floppy->fd_sector_num, floppy->fd_side, current_disk + 'A');
  ...
  if (floppy->fd_side)
    {				/* if side not zero, using one of tracks 36-70 */
      ...
      store_sect_num =
	((((floppy->fd_max_tracks * 2) - 1) - floppy->fd_track_num) * 10) + floppy->fd_sector_num;
    }
  else
    {
...
check_sector_mark_transition ()
{
  if (floppy->fd_sector_mark != floppy->fd_sector_mark_prev)
	...
	{
	  xlog (FDC, "^^^^^^^^^^^^^^^^^^^ Sector-Mark LOW -->  HIGH Transition ^^^^^^^^^^^^^^^^^^^^^^^^^\n");
	}
      else
	{
	  xlog (FDC, "VVVVVVVVVVVVVVVVVVV Sector-Mark HIGH --> LOW  Transition VVVVVVVVVVVVVVVVVVVVVVVVV\n");
	}
...
floppy_state ()
{
  ...
  xlog (FDC, "floppy_state: fdc_state = %d    fdc_state_counter = %d   cmd_ack_counter = %d\n",
