/*  Z80 instruction set disassembler.              */


/*     This file is part of  ADE - North Star Emulator,
    copyright 1995-2008 Jack Strangio)
     dasmz80 Copyright (C) Marat Fayzullin 1995-1999

************************************************************************
 99.9% of North Star Software was 8080-compatible in that it was restricted
to using only the 8080 instruction set. This was probably a throw-back to
the time when the North Star Disk System was sold as a stand-alone item and
most of their customers were using 8080 machines.
*************************************************************************
...
static char *CB_instructions[256] = {
...
static char *ED_instructions[256] = {
...
static char *plus_instructions[256] = {
...
static char *CB_plus_instructions[256] = { "RLC B", "RLC C", "RLC D", "RLC  E",
  "RLC H", "RLC L", "RLC (I%@h)", "RLC A", "RRC B", "RRC C", "RRC D",
  "RRC E", "RRC H", "RRC L", "RRC (I%@h)", "RRC A", "RL B", "RL C", "RL D",
  "RL E", "RL H", "RL L", "RL (I%@h)", "RL A", "RR B", "RR C", "RR D",
  "RR  E",
  "RR H", "RR L", "RR (I%@h)", "RR A", "SLA B", "SLA C", "SLA D", "SLA E",
  "SLA H", "SLA L", "SLA (I%@h)", "SLA A", "SRA B", "SRA C", "SRA D",
  "SRA E", "SRA H", "SRA L", "SRA (I%@h)", "SRA A", "SLL B", "SLL C", "SLL D",
  "SLL E", "SLL H", "SLL L", "SLL (I%@h)", "SLL A", "SRL B", "SRL C",
  "SRL D", "SRL E", "SRL H", "SRL L", "SRL (I%@h)", "SRL A", "BIT 0,B",
  "BIT 0,C",
  "BIT 0,D", "BIT 0,E", "BIT 0,H", "BIT 0,L",
  "BIT 0,(I%@h)", "BIT 0,A",
  "BIT 1,B", "BIT 1,C", "BIT 1,D", "BIT 1,E", "BIT 1,H", "BIT 1,L",
  "BIT 1,(I%@h)", "BIT 1,A",
  "BIT 2,B", "BIT 2,C", "BIT 2,D", "BIT 2,E", "BIT 2,H", "BIT 2,L",
...
{
  char outbuff[128], H[20], index_reg, *T, *position_ptr;
  ...
  if ((position_ptr = strchr (outbuff, '%')))
    {
      *position_ptr = index_reg;
    }

  xstate = 0;
  if ((position_ptr = strchr (outbuff, '*')) != NULL)
    xstate = 1;
  if ((position_ptr = strchr (outbuff, '@')) != NULL)
    xstate = 2;
  if ((position_ptr = strchr (outbuff, '#')) != NULL)
...
void
construct_z80_line (char *z80_machine_line, char *registers, BYTE pc0, BYTE pc1, BYTE pc2, BYTE pc3, WORD pcc)
{
  BYTE iarray[4];
  char pc_number[5];
  char opcodes_string[25];
  char instruction_codes[20];
  ...
  numbytes = DAsm (pcc, opcodes_string, iarray);

/*construct PC counter*/

  sprintf (pc_number, "%04X", pcc);

/*construct instruction codes */

  switch (numbytes)
    {
    case 1:
      sprintf (instruction_codes, " %02X       ", pc0);
      break;

    case 2:
      sprintf (instruction_codes, " %02X%02X     ", pc0, pc1);
      break;
    case 3:
      sprintf (instruction_codes, " %02X%02X%02X   ", pc0, pc1, pc2);
      break;
    case 4:
      sprintf (instruction_codes, " %02X%02X%02X%02X ", pc0, pc1, pc2, pc3);
    default:
      break;
    }
  ...
  sprintf (z80_machine_line, "%s %s %s %s", pc_number, instruction_codes, registers, opcodes_string);
