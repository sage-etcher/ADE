...
POP (WORD stack)
{
  ...
  data = (WORD) y + (WORD) (RAM (stack) * 0x100);
...
dfd_prefix (U_INT IXY)
{
  ...
  switch (op = GetBYTE (PC++ & 0x0ffff))
	...
	(cbits & 0x10) | (((cbits >> 6) ^ (cbits >> 5)) & 4) | ((cbits >> 8) & 1);
      break;
    ...
    case 0x94:			/* SUB IXYH */
      ...
      AF =
	((sum & 0xff) << 8) | (sum & 0xa8) | (((sum & 0xff) == 0) << 6) |
	(cbits & 0x10) | (((cbits >> 6) ^ (cbits >> 5)) & 4) | 2 | ((cbits >> 8) & 1);
      break;
    case 0x95:			/* SUB IXYL */
      temp = lreg (IXY);
      acu = hreg (AF);
  ...
  while (!stopsim)
    {
      ...
      /* We need to poll for X Input-Events periodically.  That also
      ...
      else
	{
      ...
      /* Now see if any outpout stuff available */
/*      if (jq_query (kbfocus))
	{
	  process_key_queue ();
	}
*/
      /* execute (both) Z80 processor(s) - one instruction */
      cpux = (&cpu);
...
{
  BYTE kf, port, data;
  static char registers[TERM_WIDTH_WIDE];
  static char z80_machine_line[TERM_WIDTH_WIDE];
...
/*construct registers display*/
	...
	flags[4] = 'C';
      ...
      sprintf (registers,
	       ...
	       hreg (AF), flags,
	       hreg (BC), lreg (BC), hreg (DE), lreg (DE), hreg (HL), lreg (HL), cpux->ix, cpux->iy,
	       RAM (HL), SP);


      construct_z80_line (z80_machine_line, registers, RAM (PC),
			  RAM ((PC + 1) & 0x0ffff), RAM ((PC + 2) & 0x0ffff), RAM ((PC + 3) & 0x0ffff), PC);
...
/********************************************************************************************/
  ...
  switch (RAM ((PC++ & 0x0FFFF)))
	...
	(((temp & 0xff) == 0) << 6) | (((temp & 0xf) == 0) << 4) | ((temp == 0x80) << 2);
      break;
    case 0x35:			/* DEC (HL) */
      temp = GetBYTE (HL) - 1;
      PutBYTE (HL, temp);
      AF = (AF & ~0xfe) | (temp & 0xa8) |
	(((temp & 0xff) == 0) << 6) | (((temp & 0xf) == 0xf) << 4) | ((temp == 0x7f) << 2) | 2;
      break;
    case 0x36:			/* LD (HL),nn */
	...
	{
	  z80_os_interface (&AF, &BC, &DE, &HL, &cpux->ix, &cpux->iy, &PC, &SP);
	}
      else
	{
	...
	case 0x44:		/* NEG */
	  ...
	  AF |=
	    ...
	    (((temp & 0x0f) != 0) << 4) | ((temp == 0x80) << 2) | 2 | (temp != 0);
	  break;
	case 0x45:		/* RETN */
	  IFF |= IFF >> 1;
	  PC = POP (SP);
	...
	case 0xA1:		/* CPI */
	  ...
	  AF =
	    ...
	    (((sum -
	       ((cbits & 16) >> 4)) & 2) << 4) | (cbits & 16) | (sum & 8) | ((--BC & 0xffff) != 0) << 2 | 2;
	...
	case 0xB1:		/* CPIR */
	  ...
	  AF =
	    (AF & ~0xfe) | (sum & 0x80) | (!(sum & 0xff) << 6) |
	    (((sum - ((cbits & 16) >> 4)) & 2) << 4) | (cbits & 16) | (sum & 8) | op << 2 | 2;
	  if ((sum & 15) == 8 && (cbits & 16) != 0)
	    AF &= ~8;
	  break;
	case 0xB2:		/* INIR */
...
/* BREAK ADDRESS STUFF. Stops execution of Z80. Start up monitor CLI. cancels break-point. */
  ...
  if ((PC & 0xffff) == break_address)
    {
      xlog (ALL, " break - address = %06X \n", PC);
      stopsim = 1;
      cpux->break_address = 0x01ffff;	/*reset break-point to beyond top of memory = inactive */
      cpux->debug = break_dbg;	/* set auto debugging level if not on before */
    }
...
/* Z80 instructions continue one normally.                                          */
  ...
  if ((PC & 0xffff) == cpux->trap_address)
    {
      xlog (TRAP, "\ntrap_address=%08x\n", cpux->trap_address);
  ...
  if (((PC & 0xffff) == 0x08483) && (blanking_flag))
    {
      screen_log (AF / 256);
...
/* make registers visible for debugging if Z80 Emulation interrupted */
/*  SAVE_STATE (); */
  return (PC & 0xffff) | 0x10000;	/* flag non-bios stop */
}
...
set_z80_interrupt (int hwint)	/* hi-byte: mode 2 offset; lo-byte VI number */
{
  ...
  switch (interrupt_mode)
    ...
    case 2:
      ...
      xlog (DEV, "mode_2_interrupt: vector %d: address of new PC storage = %04X\n", vector_num,
	    interrupt_vector);
      cpux->interrupt_newpc = (ram[interrupt_vector + 1] * 256) + ram[interrupt_vector];
      xlog (DEV, "mode_2_register_interrupt:  offset = %04x ir= %04X vector = %04X  new PC address= %04X\n",
...
mode_2_register_interrupt (BYTE mode_2_offset)
{
  ...
  xlog (DEV,
	...
	cpux->mode_2_offset, cpux->ir, interrupt_vector, cpux->interrupt_newpc);
}
